...
export const header = (element: HTMLElement,quizz?: Quizzes) :void=>{
  ...
  if(quizz === undefined || quizz === null){
    ...
    titleElement = `<span id=${quizz.title} class="rounded-lg w-[2.4rem] h-[2.4rem] flex justify-center"><img src="${quizz?.icon}" alt="${quizz?.title}"></span> <h1 class="text-3xl font-bold tracking-wide">${quizz?.title}</h1>`
  }
  element.innerHTML = `<span class="titleQuizz flex items-center gap-4">
        ${titleElement}
      </span>
